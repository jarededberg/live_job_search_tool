...
## Salary insight pages

`/salary`, `/salary/<role-slug>`, and `/salary/company/<company-slug>`
are real, crawlable pages aggregating this site's own scraped
`salary_min`/`salary_max` data — median, full range, and sample size —
per canonical role family and per company. Entirely computed live from
current postings, not a survey or third-party data source, and the
pages say so.

Role classification is a new `role_groups.py` module, deliberately
separate from `role_synonyms.py`: the synonyms module expands a resume
phrase into a list of *related* search terms (many-to-many, tuned for
recall), while `role_groups.py`'s `classify_role(title)` puts a job
title into exactly *one* canonical bucket (same ordered-keyword-match
pattern as `department_groups.py`), because a salary rollup needs every
job to land in a single, stable bucket to aggregate correctly. 22
curated role families (Software Engineer, Product Manager, Account
Executive, ...); a title that matches none of them just isn't included
in any role-level rollup (still counts toward its company's rollup).

`db.py`'s `salary_stats_by_role()`/`salary_stats_for_role()`/
`salary_stats_by_company()`/`salary_stats_for_company()` share one
`MIN_SALARY_SAMPLE_SIZE = 3` cutoff: a role or company with only one or
two disclosed salaries doesn't get a page (404s, same "don't publish a
thin/misleading page" reasoning as `company_hub_page()`'s
no-current-openings case) — not enough of a sample to present as a real
market figure. Median is computed on each job's own `(min+max)/2`
midpoint, not by separately medianing mins and maxes against each
other.

Cross-linked from `/jobs/company/<slug>` (a "See `<Company>` salary
data →" link, shown only when that company clears the sample-size
cutoff) and from the homepage footer. Has its own `/sitemap-salary.xml`
(role pages are a fixed ~22 URLs; company pages scale with the dataset,
...
Before this round, the site had essentially none: no meta descriptions,
no Open Graph/Twitter Card tags (so links shared on Slack/social/iMessage
rendered with nothing useful), no sitemap, and no robots.txt of its own
(Cloudflare was serving its own generic default in front of it). This is
tier one -- fast, low-risk wins that don't touch the app's architecture:

- **Meta descriptions + canonical link tags** on all four public pages
  (`/`, `/about`, `/faq`, `/contact`) -- each is unique and specific to
  that page's actual content, not a copy-pasted site-wide blurb.
- **Open Graph + Twitter Card tags** on the same four pages (`og:title`,
  `og:description`, `og:image`, `twitter:card`, etc.), so links shared
  anywhere render with a real title, description, and image instead of a
  bare URL. `static/og-image.png` (1200×630, generated once via Pillow
  from a DejaVu Sans font, matching the site's maroon/cream branding) is
  the shared image for all four -- one generic branded card is enough at
  this stage; page-specific share images aren't worth the added
  complexity yet.
- **`GET /robots.txt`** (`app.py`) -- a real one, generated by the app
  instead of Cloudflare's placeholder. Allows everything except
  `/admin` (already `noindex` via its own `<meta>` tag -- this is
  defense in depth, not the only thing keeping it out of search results),
  the raw `/api/` JSON endpoints (not pages, nothing there should be
  indexed), and `/reset-password` (carries a single-use token in the
  URL). Points crawlers at `/sitemap.xml`.
- **`GET /sitemap.xml`** (`app.py`) -- lists the four public pages with
  `changefreq`/`priority` hints. Both routes use a new `SITE_URL` constant
  (`app.py`, right under `APP_BASE_URL`) rather than building URLs from
  `request.url_root` -- this app has no `ProxyFix` configured, and
  Render/Cloudflare terminate TLS in front of it, so `request.url_root`
  can't be trusted to reliably report `https://`. `SITE_URL` reuses
  `APP_BASE_URL` if that's already set, otherwise falls back to the
